            ...
            <div class="col-lg-8">
                <div class="htc__blog__details__wrap">
                    <div class="ht__comment__form">
                        ...
                        <form class="form-horizontal" action="qna-form2.do" method="post">
                            <input type="hidden" name="customerTbNo" value="${sessionScope.customer.customerTbNo}">
                            <input type="hidden" name="qnaTbRef" value="${replyFrom.qnaTbRef}">
                            <input type="hidden" name="qnaTbStep" value="${replyFrom.qnaTbStep}">
                            <input type="hidden" name="qnaTbDepth" value="${replyFrom.qnaTbDepth}">
                                ...
                                <div class="col-sm-8">
                                    <select name="qnaTypeTbNo" id="qnaTypeTbname" class="form-control1">
                                        <c:forEach var="category" items="${qnaCategoryList }">
											<c:if test="${category.qnaTypeTbNo eq replyFrom.qnaTypeTbNo}">
												<option value="${category.qnaTypeTbNo }" selected>${category.qnaTypeTbName }</option>
											</c:if>
											<c:if test="${category.qnaTypeTbNo ne replyFrom.qnaTypeTbNo}">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-8"><textarea id="qnaTbContent" placeholder="Context *" name="qnaTbContent"><c:if test="${replyFrom.pdSaleTbNo ne null}">제품판매코드: ${replyFrom.pdSaleTbNo}</c:if><c:if test="${replyFrom.pdOrderTbNo ne null}">주문상품번호: ${replyFrom.pdOrderTbNo}</c:if></textarea></div>
                            </div>
                            <div class="form-group">
